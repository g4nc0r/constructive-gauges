...
Spearbit's November 2023 security review of the Slipstream concentrated-liquidity implementation~\cite[\S5.3.1]{spearbit2023} noted that the reward accumulator may be gameable on chains with a mempool, described a claim-unstake-JIT-restake loop, and recommended evaluating whether a volume-based reward system would be fairer than the time-based accumulator; the issue was acknowledged, with monitoring rather than redesign proposed. The present work formalises a more general strategy space that does not require mempool visibility (\S\ref{sec:empirical}), demonstrates via the operational dataset of~\S\ref{sec:calibration} that gaming is economically viable at production scale, and specifies a corrective scoring function tracking tradeable depth.
...
Second, the remediations are ad hoc checks rather than an articulation of what the gauge is measuring. A framework that targets a proxy for tradeable depth closes each classified pathway (Theorem~\ref{thm:mincomplete}) rather than enumerating specific attack patterns; we do not claim coverage of attack variants outside the three-pathway classification of Definition~\ref{def:pathways}.
...
Theorem~\ref{thm:mincomplete} is a characterisation of the abstract family $\mathcal{F}$: the three corrections are not substitutes, and $S^{\{c, f, \mathbf{1}\}}$ is the minimal element closing all three pathways. Scoring functions outside $\mathcal{F}$ (e.g., with volume-coupled multipliers) fall outside this result.
...
On-chain decoding of an Aerodrome extraction pattern observed in April 2026 shows that Flashblocks sub-block ordering compresses the operator's wall-clock impermanent-loss exposure per rebalance cycle. Withdrawals are sequenced early within a parent block and deposits late, so the operator is unstaked for most of the block's flashblocks. The Synthetix-style accumulator advances on \texttt{block.timestamp} deltas between parent blocks, so per-cycle reward capture is one parent-block's worth of emissions regardless of sub-block positioning. Flashblocks amplifies the ratio of reward collected to IL exposure while leaving the reward basis unchanged. Each parent-block boundary a stake crosses credits a full block of emissions regardless of whether any swap was facilitated; without Flashblocks the same strategy runs at 1:1 leverage but the gap remains.

Under the corrective framework, the freshness factor $f(t - t_i)$ suppresses short cycles independently of sub-block ordering ($\bar f(\delta t) = \delta t/(2W)$ for $\delta t \leq W$), and the concentration factor $c(w)$ addresses width-independence (property~(2)). In the observed Aerodrome extraction pattern, $93.6\%$ of staked blocks across $15$ targeted pools contained zero swap events and every position except one used minimum tick spacing; both observations characterise the strategy rather than the Flashblocks execution. The BSC and Arbitrum fork replications of~\S\ref{sec:testmap} reproduce the same framework behaviour on chains without Flashblocks.
\end{remark}
...
Claims of~\S\ref{sec:framework} are mapped to Foundry tests deployed against live Aerodrome Slipstream state on Base mainnet (WETH/USDC CL100 pool at \texttt{0xb2cc224c1c9feE385f8ad6a55b4d94E92359DC59}), using the production NFPM for minting and AERO as the emission token. Theorem~\ref{thm:parasitic}'s empirical component is validated separately in~\S\ref{sec:empirical-pin}.
...
Merkl by Angle Labs~\cite{merkl2023} is the closest deployed analogue. It operates at the reward-distribution layer with off-chain computation and optimistic verification, while the corrective framework operates on-chain at the gauge scoring function with deterministic verification. Merkl's weights are also independently configurable per campaign without a minimality argument that all three must be non-zero; Theorem~\ref{thm:mincomplete} provides that minimality here. Merkl has been used to supplement gauge emissions but not to replace the gauge formula, because ve(3,3)'s voter-fee flow and veNFT voting remain bound to the on-chain per-pool accumulator.

No deployed gauge inspected for this paper composes graded concentration, freshness, and an in-range factor at the LP-measurement layer, and none offers a minimality argument for the joint necessity of the three analogous to Theorem~\ref{thm:mincomplete}.

% ---- 8. Conclusion ----
\section{Conclusion}\label{sec:conclusion}

Against a concentrated-liquidity gauge baseline, under the Gaussian log-price model and large-$N$ mean-field conditions, the corrective scoring function of~\S\ref{sec:framework} bounds parasitic extraction by the product of a freshness ratio and a concentration ratio (Theorem~\ref{thm:parasitic}) and imposes bounded drag on legitimate LPs (Theorem~\ref{thm:legit}). The scoring function with all three corrections is the unique minimal member of the abstract product family closing all three parasitic pathways (Theorem~\ref{thm:mincomplete}); on CL gauge deployments the indicator is already supplied by the baseline accumulator, leaving $c$ and $f$ as the novel corrections. Cross-chain fork tests against PancakeSwap V3, Thena FUSION, Ramses V2, and a reference implementation against live Aerodrome Slipstream (\S\ref{sec:empirical}), empirical validation of the in-range probability formula against live tick traces (\S\ref{sec:empirical-pin}), and calibration against an operational dataset of $94{,}469$ rebalance events (\S\ref{sec:calibration}) are consistent with the analysis and ground the parameter choices.
...
Spearbit (2023) `Velodrome Finance Security Review', \textit{Spearbit Portfolio}, November 2023. Available at: \url{https://github.com/spearbit/portfolio/raw/master/pdfs/Velodrome-Spearbit-Security-Review-Nov23.pdf}.
...
\begin{enumerate}[leftmargin=2em]
\item Pull all \texttt{Swap} events from the target pool over a $24$-hour window (block range $[\text{head} - 43200, \text{head}]$ on Base, with $2$-second blocks). Decode the tick from the fifth $32$-byte word of each event's data as a sign-extended \texttt{int24}.
...
\item For each $(\delta t, w)$ pair, uniformly sample $N_s = 1500$ starting blocks from the trace. For each sample $s$, compute the in-range fraction of $[s, s + \delta t / 2]$ for a position of half-width $w/2$ centred at $\tau(s)$.
...
The repository is at \url{https://github.com/g4nc0r/constructive-gauges}; the Zenodo DOI \url{https://doi.org/10.5281/zenodo.19690075} resolves to an immutable archival snapshot. The reference implementation realises equation~(\ref{eq:score}) with the parameter choices of Table~\ref{tab:reference} on a minimal Synthetix-style accumulator, and reproduces the fork-test results in Table~\ref{tab:forkmap}. The suite runs against the Aerodrome Slipstream WETH/USDC CL100 pool on Base mainnet (\texttt{0xb2cc224c1c9feE385f8ad6a55b4d94E92359DC59}), with cross-chain replications on BSC and Arbitrum One. Reproduction from a clean checkout:
